...
class VQADataset(torch.utils.data.Dataset):
    def __init__(self, df_path, image_dir, transform=None, answer=True):
        ...
        # class mappingの単語を追加
        class_mappings = pandas.read_csv("https://huggingface.co/spaces/CVPR/VizWiz-CLIP-VQA/raw/main/data/annotations/class_mapping.csv")
        for class_mapping in class_mappings["answer"]:
            words = class_mapping.split(" ")
            for word in words:
                word = process_text(word)
                if word not in self.question2idx:
                    self.question2idx[word] = len(self.question2idx)
...
def main():
    ...
    model.load_state_dict(torch.load('1/model.pth'))
    ...
    scheduler = CosineScheduler(num_epoch, lr=lr, warmup_length=warmup_length)
    ...
    for epoch in range(num_epoch):
        ...
        #model_scripted.save(f'model_scripted{epoch+1}.pth')
        #torch.save(model.state_dict(), f'model_weight{epoch+1}.pth')
